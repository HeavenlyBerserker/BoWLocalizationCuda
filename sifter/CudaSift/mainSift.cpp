...
void ExtractSiftDescriptors (const char * imageName, float blurFactor, float threshold, int deviceNumber)
{
  ...
  CudaImage imgCuda;
  imgCuda.Allocate(width, height, iAlignUp(width, 128), false, NULL, (float *)img.data);
  imgCuda.Download();
...
{   
  ...
  unsigned int w1 = img007.cols;
  unsigned int h1 = img007.rows;
  std::cout << "Image size = (" << w1 << "," << h1 << ")" << std::endl;
  
  InitCuda(0);
  CudaImage img_1;
  img_1.Allocate(w1, h1, iAlignUp(w1, 128), false, NULL, (float*)img007.data);
  img_1.Download();
  
  SiftData siftData_1;
  float initBlur_1 = 1.0f;
  float thresh_1 = 1.0f; 
  InitSiftData(siftData_1, 32768, true, true); 

  //for ( thresh_1 = 1.00f; thresh_1 <= 4.01f; thresh_1 += 0.50f ) {
    for ( int i = 0; i < 10; i++ ) {
      ExtractSift(siftData_1, img_1, 5, initBlur_1, thresh_1, 0.0f, false);
    }

    //std::cout << "Number of original features: " <<  siftData_1.numPts << std::endl;
  //}
  
  std::cout << siftData_1.numPts << std::endl;
  
  for ( int i = 0; i < siftData_1.numPts; i++ ) {
    for ( int j = 0; j < 128; j++ ) {
      std::cout << siftData_1.h_data[i].data[j] << " ";
    }
    std::cout << std::endl;
  }
...
/*
  ...
  for ( int i = 0; i < siftData1.numPts; i++ ) {
    for ( int j = 0; j < 128; j++ ) {
      std::cout << siftData1.h_data[i].data[j] << " ";
    }
    std::cout << std::endl;
  }
